...
- **알람 기능 (v1.1.0)**:  
  - 지정된 시간에 트레이 알람 팝업 표시 (최소화 상태에서도 작동)  
  - 알람 메시지 커스터마이징 및 **프리셋 1,2,3** 지원  
  - Apply 버튼으로 즉시 적용, Reset Current 버튼으로 선택된 프리셋 초기화  
  - 프리셋 상태 및 메시지 자동 동기화 → 앱 재시작 후에도 유지

## 🚀 시작하기

### 실행 파일 (Windows 전용)
파이썬 설치 없이 바로 사용하려면 [Releases](https://github.com/dnrjke/UTCtoKST/releases) 페이지에서 `UTCtoKST.exe`를 다운로드하여 실행하세요.
